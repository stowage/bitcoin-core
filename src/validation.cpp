...
CBlockIndex * BlockManager::InsertBlock(const uint256& hash) EXCLUSIVE_LOCKS_REQUIRED(cs_main){ return this->InsertBlockIndex(hash); }
...
{
    auto insblock = [this](const uint256& hash) { return this->InsertBlock(hash); };
    std::function<CBlockIndex *(const uint256 &)> insblock2 = insblock;
    if (!blocktree.LoadBlockIndexGuts(consensus_params, insblock2))
        return false;
