...
UniValue importpubkey(const JSONRPCRequest& request)
{
...

    {
        ...
        pwallet->ImportPubKeys({pubKey.GetID()}, {{pubKey.GetID(), pubKey}} , std::map<CKeyID, std::pair<CPubKey, KeyOriginInfo>>{} /* key_origins */, false /* add_keypool */, false /* internal */, 1 /* timestamp */);
    }
    if (fRescan)
    {
        RescanWallet(*pwallet, reserver);
